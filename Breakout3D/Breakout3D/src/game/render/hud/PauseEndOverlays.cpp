/**
 * @file PauseEndOverlays.cpp
 * @brief Overlays de ecrã inteiro para estados não-jogáveis: Pause, Game Over, Win e escolha de cartas Rogue.
 *
 * @details
 *  Este ficheiro desenha *sempre por cima de tudo* (HUD, danger zone, FX, etc.).
 *  A função principal @ref drawPauseOrEndOverlay deve ser chamada no fim do frame.
 *
 *  Estados cobertos:
 *   - GameMode::ROGUE_CARDS  -> overlay de escolha de cartas (3 cards + botão PICK)
 *   - GameMode::PAUSED      -> painel de pausa (e, no Rogue, mostra também as cartas escolhidas + tooltip)
 *   - GameMode::GAME_OVER   -> painel de fim de jogo (botões RETRY/MENU)
 *   - GameMode::WIN         -> painel de vitória (botões RETRY/MENU)
 *
 *  Nota importante sobre “mask overlay”:
 *   - Usamos drawUIQuad fullscreen com máscara (useMask=true) para:
 *       * escurecer o ecrã todo
 *       * deixar a área do painel “limpa” / sem dim por cima
 *     Isto cria o look “foco no painel” sem ter de desenhar recortes complexos.
 *
 *  Nota sobre o estilo neon/RGB:
 *   - Títulos e borders usam HSV->RGB com sin(time) para um efeito vivo.
 *   - Há glow (várias passagens com offsets) + outline (4 direcções).
 */
 ...
 /**
  * @brief Desenha um título com estilo “neon RGB”: glow + outline + cor por letra.
  *
  * @param r        Renderer de UI.
  * @param title    Texto completo do título.
  * @param x        Posição X (canto inferior-esquerdo do texto, em px).
  * @param y        Posição Y (canto inferior-esquerdo do texto, em px).
  * @param scale    Escala do texto (UI units).
  * @param timeNow  Tempo actual (segundos) para animar o hue.
  *
  * @details
  *  Implementação em 3 camadas:
  *   1) Glow: desenha o texto 4 vezes (left/right/up/down) com offsets grandes, alpha pequena
  *   2) Outline: por letra, 4 offsets (2px) com uma cor escura para “recorte”
  *   3) RGB: por letra, escolhe hue ligeiramente diferente para fazer gradiente ao longo do texto
  */
 static void drawRgbTitle(engine::Renderer& r,
                          ...
                          float timeNow) {
     // -------------------------------------------------------------------------
     // 1) Glow: várias passagens com offsets decrescentes.
     // Quanto maior o offset, menor a alpha, para não “lavar” o texto.
     // -------------------------------------------------------------------------
     ...
     // -------------------------------------------------------------------------
     // 2) Render por letra:
     //    - para posicionar cada letra, mede a largura do prefixo (0..i-1)
     //    - desenha outline (4 passes)
     //    - desenha cor RGB final
     //
     // Nota: substr em loop não é o mais barato, mas aqui é UI (poucos chars)
     // e só aparece em overlays (não é a cada frame de gameplay intenso).
     // -------------------------------------------------------------------------
     ...
     for (int i = 0; i < n; ++i) {
         ...
         // Outline (escuro) para separar do fundo e ficar legível
         ...
         // RGB: hue varia ao longo do texto + “wave” temporal para animação
         float hue = std::fmod(
             0.56f +
             (float)i / (float)std::max(1, n - 1) * 0.35f +
             0.08f * std::sin(timeNow * 1.2f + i * 0.3f),
             1.0f
         );
 ...
 /**
  * @brief Desenha o overlay apropriado quando o jogo não está em PLAYING.
  *
  * @details
  *  Prioridade (por ordem):
  *   1) Se PLAYING -> não desenha nada.
  *   2) Se ROGUE_CARDS -> desenha overlay de escolha de cartas e retorna.
  *   3) Se PAUSED -> desenha overlay de pausa (com cards Rogue se aplicável) e retorna.
  *   4) Caso contrário -> GAME OVER ou WIN (end overlay).
  *
  *  Isto garante que só um overlay é desenhado por frame (sem conflitos).
  */
 void drawPauseOrEndOverlay(const RenderContext& ctx, const GameState& state) {
     // -------------------------------------------------------------------------
     // Overlays só fazem sentido fora de PLAYING.
     // -------------------------------------------------------------------------
     if (state.mode == GameMode::PLAYING) return;
 
     // =========================================================================
     // (A) ROGUE CARD PICK OVERLAY
     // =========================================================================
     if (state.mode == GameMode::ROGUE_CARDS) {
         // Layout calculado centralmente (posições e tamanhos dos 3 cards).
         const auto L = game::ui::rogueCardOverlay(ctx.fbW, ctx.fbH);
 
         // ---------------------------------------------------------------------
         // Dim fullscreen: sem “painel gigante”, só um escurecimento suave
         // para focar a atenção nos cards.
         // ---------------------------------------------------------------------
         ctx.renderer.drawUIQuad(0, 0, (float)ctx.fbW, (float)ctx.fbH, glm::vec4(0, 0, 0, 0.62f));
 
         // ---------------------------------------------------------------------
         // Helper: tornar descrições mais “UX-friendly”.
         // Converte linhas começadas por '+' ou '-' em:
         //   +... -> "GAIN: ..."
         //   -... -> "COST: ..."
         //
         // Isto melhora legibilidade porque o jogador lê “GAIN/COST” instantaneamente.
         // ---------------------------------------------------------------------
         auto prettifyDesc = [](const std::string& raw) -> std::string {
             ...
             // remove trailing newline se existir
             if (!out.empty() && out.back() == '\n') out.pop_back();
             return out;
         };
 
         // ---------------------------------------------------------------------
         // drawCard: desenha um card completo (shadow, body, border, name, desc e botão PICK).
         //
         // Parâmetros:
         //  - r: rect do card
         //  - id: card id (para ir buscar def e accent)
         //  - hovered: se o rato está por cima do card (efeitos mais fortes)
         //
         // Nota: o “hoveredRogueCardPickButton” é uma verificação separada
         // para pôr RGB border apenas no botão PICK, quando o mouse está no botão.
         // ---------------------------------------------------------------------
         auto drawCard = [&](const game::ui::Rect& r, game::rogue::RogueCardId id, bool hovered) {
             ...
             float uiS = 1.0f; // se um dia quiseres “UI global scale”, é aqui que encaixa
 
             // Shadow: deslocado em baixo/direita (y é bottom-up, por isso -sh “desce”)
             float sh = hovered ? 20.0f * uiS : 10.0f * uiS;
             ctx.renderer.drawUIQuad(r.x + sh, r.y - sh, r.w, r.h,
                                     glm::vec4(0, 0, 0, hovered ? 0.66f : 0.55f));
 
             // Card body: match menu vibe (quase preto azul)
             glm::vec4 body(0.08f, 0.08f, 0.14f, 0.98f);
             ctx.renderer.drawUIQuad(r.x, r.y, r.w, r.h, body);
 
             // Border:
             //  - hovered -> RGB animado (mais “premium”)
             //  - else    -> cor acento da carta
             ...
             // -----------------------------------------------------------------
             // Nome do card (centrado em cima)
             // - fitScaleToWidth: evita overflow do nome
             // -----------------------------------------------------------------
             ...
             // -----------------------------------------------------------------
             // Descrição (wrapped)
             // - usamos uma coluna com padding (padX)
             // - ui::drawWrappedText controla quebras e espaçamento
             // -----------------------------------------------------------------
             std::string desc = prettifyDesc(def.shortDesc);
             float padX = 34.0f * uiS;
             float dScale = 1.18f * uiS;
             glm::vec4 dCol(0.93f, 0.97f, 1.0f, 0.95f);
 
             float dTop = ny - 140.0f * uiS;
             ui::drawWrappedText(ctx.renderer,
                                 r.x + padX, dTop,
                                 r.w - 2.0f * padX,
                                 desc, dScale, dCol,
                                 12.0f * uiS);
             ...
             // -----------------------------------------------------------------
             // Botão PICK (estilo “play mode card”)
             // - fica em baixo, centrado
             // - se hovered card -> botão fica mais brilhante e com shadow maior
             // -----------------------------------------------------------------
             float pbW = r.w * 0.56f;
             float pbH = 76.0f * uiS;
             ...
             ctx.renderer.drawUIQuad(pbX + btnShadow, pbY - btnShadow, pbW, pbH,
                                     glm::vec4(0,0,0, hovered ? 0.55f : 0.45f));
             ctx.renderer.drawUIQuad(pbX, pbY, pbW, pbH, glm::vec4(btnCol, 1.0f));
 
             // RGB border ONLY se o hover está mesmo no botão PICK daquele card
             if (state.hoveredRogueCardPickButton >= 0 &&
                 state.hoveredRogueCardPickButton < state.rogueOfferCount &&
         ...
         // desenhar até 3 cards (dependendo do offerCount)
         ...
         // ---------------------------------------------------------------------
         // Título “CHOOSE A CARD” no topo, no mesmo slot do HUD do Rogue.
         // Assim fica consistente e “não mexe” com layout mental do jogador.
         // ---------------------------------------------------------------------
         {
             ...
             float y = (float)ctx.fbH - 18.0f - th;
     ...
     if (state.mode == GameMode::PAUSED) {
         // Em Rogue, se tiver muitas cartas, usa layout maior.
         ...
         // ---------------------------------------------------------------------
         // Fullscreen dim com máscara:
         // - fora do painel: escurece pesado
         // - dentro do painel: fica “limpo” (sem dim por cima)
         // ---------------------------------------------------------------------
         glm::vec2 maskMin(panelX2, panelY2);
         glm::vec2 maskMax(panelX2 + panelW2, panelY2 + panelH2);
         ctx.renderer.drawUIQuad(0, 0, (float)ctx.fbW, (float)ctx.fbH,
                                 glm::vec4(0, 0, 0, 0.82f),
                                 true, maskMin, maskMax);
 
         // fundo do painel (opaco)
         ctx.renderer.drawUIQuad(panelX2, panelY2, panelW2, panelH2,
                                 glm::vec4(0.06f, 0.06f, 0.06f, 1.0f));
 
         // border neon RGB do painel (consistência com títulos)
         float bt2 = 3.0f;
         ...
         // ---------------------------------------------------------------------
         // Título “PAUSED” com glow + per-letter gradient (igual a game over/win)
         // ---------------------------------------------------------------------
         ...
         for (int i = 0; i < nP; ++i) {
             ...
             float hueP = std::fmod(
                 0.56f +
                 (float)i / (float)std::max(1, nP-1) * 0.35f +
                 0.08f * std::sin(tP * 1.2f + i * 0.3f),
                 1.0f
             );
         ...
         // ---------------------------------------------------------------------
         // Secção extra: mostrar cartas escolhidas no ROGUE
         // - grid de badges (abbrev) em 2 rows
         // - tooltip/descrição se hovered
         // ---------------------------------------------------------------------
         if (state.gameType == GameType::ROGUE && !state.rogueChosen.empty()) {
             ...
             // -------------------------------------------------------------
             // Grid de badges:
             // - max 20 cards para não partir layout
             // - 10 colunas x 2 rows
             // - badgeRowGap maior para dar espaço ao label/tooltip
             // -------------------------------------------------------------
             float badgeStartY = titleY - 80.0f;
             float badgeBaseW = 50.0f;
             float badgeBaseH = 50.0f;
             float badgeGap = 10.0f;
             float badgeRowGap = 80.0f;
             float badgesPerRow = 10;
             ...
             for (size_t i = 0; i < state.rogueChosen.size() && cardCount < 20; ++i) {
                 ...
                 // hovered: usado para borda mais grossa e para mostrar label
                 bool isHovered = (state.hoveredPauseRogueCard == (int)i);
 
                 // abreviação do card (fallback: 3 letras do nome)
                 ...
                 // shadow
                 ctx.renderer.drawUIQuad(badgeX + 3.0f, badgeY - 3.0f, badgeW, badgeH, glm::vec4(0, 0, 0, 0.5f));
 
                 // body (accent escurecido)
                 glm::vec3 badgeCol = accent * 0.7f;
                 ctx.renderer.drawUIQuad(badgeX, badgeY, badgeW, badgeH, glm::vec4(badgeCol, 0.95f));
 
                 // border (mais thick se hovered)
                 ...
                 // label (nome completo) só em hover, para não poluir o painel
                 if (isHovered) {
             ...
             // -------------------------------------------------------------
             // Tooltip: se existe um badge hovered, mostra caixa grande com nome+desc.
             // Isto dá detalhe sem ocupar espaço sempre.
             // -------------------------------------------------------------
             if (state.hoveredPauseRogueCard >= 0 &&
                 state.hoveredPauseRogueCard < (int)state.rogueChosen.size()) {
                 ...
                 float tooltipY = panelY2 + 120.0f;
 
                 // background + shadow
                 ctx.renderer.drawUIQuad(tooltipX + 4.0f, tooltipY - 4.0f, tooltipW, tooltipH, glm::vec4(0, 0, 0, 0.5f));
                 ctx.renderer.drawUIQuad(tooltipX, tooltipY, tooltipW, tooltipH, glm::vec4(0.08f, 0.08f, 0.14f, 0.95f));
 
                 // border accent
                 float borderT = 2.5f;
                 ctx.renderer.drawUIQuad(tooltipX - borderT, tooltipY - borderT, tooltipW + 2*borderT, borderT, glm::vec4(hoveredAccent, 1.0f));
                 ctx.renderer.drawUIQuad(tooltipX - borderT, tooltipY + tooltipH, tooltipW + 2*borderT, borderT, glm::vec4(hoveredAccent, 1.0f));
                 ctx.renderer.drawUIQuad(tooltipX - borderT, tooltipY, borderT, tooltipH, glm::vec4(hoveredAccent, 1.0f));
                 ctx.renderer.drawUIQuad(tooltipX + tooltipW, tooltipY, borderT, tooltipH, glm::vec4(hoveredAccent, 1.0f));
 
                 // nome do card no topo do tooltip
                 ...
                 // pequeno glow lateral (só left/right) para não borrar
         ...
         // ---------------------------------------------------------------------
         // Botões de pausa: Restart / Menu
         // - drawOverlayButton faz o estilo consistente (shadow + hover highlight)
         // - hoveredOverlayButton vem do input handling (mouse).
         // ---------------------------------------------------------------------
     ...
     for (int i = 0; i < nE; ++i) {
         ...
         float hueE = std::fmod(
             0.56f +
             (float)i / (float)std::max(1, nE-1) * 0.35f +
             0.08f * std::sin(tE * 1.2f + i * 0.3f),
             1.0f
         );
